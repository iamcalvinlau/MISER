...
def find_subsets(s, n, list_of="array"):
    import itertools
    list_of_tuples = list(itertools.combinations(s, n))
    if(list_of=='tuple'):
        return list_of_tuples
    elif(list_of=='array'):
        list_of_arrays = []
        for tup in list_of_tuples:
            list_of_arrays.append(np.array(tup))
...
class TLLSQ(BaseOptimizer):
    """Term-limited least squares algorithm.
    Attempts to minimize the objective function
    :math:`\\|y - Xw\\|^2_2 + \\alpha \\|w\\|^2_2`
    by iteratively performing least squares and masking out
    elements of the weight array w that are below a given threshold.
    ...
    def _sparse_regress(self, x, y, iterate=True):
        """Perform ridge regression for every combination and choose best"""
        kw = self.ridge_kw or {}
        indices = find_subsets(range(x.shape[1]),self.target_n_terms) 
        
        score = np.zeros(len(indices))
        coef_total = np.zeros_like(score)
        coefs = []
        # Calculate the score after every possible elimination
        for i, ind in enumerate(indices):
            # Use particular slice
            x_test = np.copy(x[:,ind])
            regr_ = Ridge(self.alpha, **kw)
            # regr_ = Lasso(self.alpha, **kw)
            # regr_ = ElasticNet(self.alpha, **kw)
            regr_.fit(x_test, y);
        ...
        # Use the coefficients of that best elimination,
        # making sure to stick a zeros in places of the eliminated coefficient
        coef = np.zeros(x.shape[1]);
        coef[indices[i_best]] = np.asarray(coefs[i_best]); 
        if(iterate):self.iters += 1
        return coef
